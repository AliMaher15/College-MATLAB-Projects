...
%% Mixer
off = "10";
offset = double(off) * power(10,3);
wif = 25000; %wIF = 25kHZ
carrier = cos(2*pi*(fc(i) + wif + offset)*t);
mt = mt.*carrier;
figure(3)
messageplot(mt,fs, "Mixing with wIF and Offset = " + off + " kHz", 1, 2, 1);
%% IF Stage
xIF = 2000;
%filter design for the wanted message
...
audiowrite(off + "kHz offset Output "+ message, mt, fs);